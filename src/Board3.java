...
public class Board3 implements Comparable<Board3> {
	...
	private Board3 parrent;
	private Point zero;
	private List<Board3> moves;
	private int[][] state;

	public Board3(char direction, int[][] state, Board3 parrent, int distance,
			Point point) {
		super();
	...
	public void setMoves() {
		...
		if (zero.getY() - 1 >= 0
			...
			Point left = new Point(zero.getX(), (byte) (zero.getY() - 1));
			int[][] newState = deepClone(state);
			swap(left, newState);
			move = new Board3(LEFT, newState, this, distance + 1, left);
			moves.add(move);
		}
		if (zero.getY() + 1 < state.length
			...
			int[][] newState = deepClone(state);
			swap(right, newState);
			move = new Board3(RIGHT, newState, this, distance + 1, right);
			moves.add(move);
		}
		if (zero.getX() - 1 >= 0
			...
			int[][] newState = deepClone(state);
			swap(up, newState);
			move = new Board3(UP, newState, this, distance + 1, up);
			moves.add(move);
		}
		if (zero.getX() + 1 < state.length
			...
			int[][] newState = deepClone(state);
			swap(down, newState);
			move = new Board3(DOWN, newState, this, distance + 1, down);
			moves.add(move);
		}
	}
